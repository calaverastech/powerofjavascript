...
describe("StockController", function() {
  ...
  describe("non-available service", function() {
    ...
    it("should give a message", function() {
      ...
      expect(scope.customResults.length).toBe(0);
    });
  });
  ...
  describe("available service", function() {
    ...
    describe("when adding a ticker", function() {
      ...
      it("should add 'WMMVF' to an empty list", function() {
        ...
        httpBackend.flush();
        expect(scope.customResults.length).toBe(1);
        expect(scope.customResults).toContain(tests.testStock1);
        expect(scope.customResults[0].Symbol).toBe('WMMVF');
      });
      
      it("should add 'WMMVF' to the non-empty list", function() {
        scope.customResults = [tests.testStock2, tests.testStock3];
        scope.$apply(function() {
          scope.show(ticker1);
        });
        httpBackend.flush();
        expect(scope.customResults.length).toBe(3);
        expect(scope.customResults).toContain(tests.testStock1);
        expect(scope.customResults[2].Symbol).toBe('WMMVF');
      });
    });
    
    describe("when clicking on 'x'", function() {
      it("should remove the ticker from the custom list", function() {
        scope.customResults = [tests.testStock1, tests.testStock2, tests.testStock3];
        expect(scope.customResults.length).toBe(3);
        scope.removeTicker(0);
        expect(scope.customResults.length).toBe(2);
        expect(scope.customResults).toEqual([tests.testStock2, tests.testStock3]);
        expect(scope.customResults).not.toContain(tests.testStock1);
      });
    });
    ...
    describe("when refreshing the custom list", function() {
      ...
      it("should get new values", function() {
        ...
        httpBackend.flush();
        expect(scope.customResults.length).toBe(2);
        expect(scope.customResults).toContain(tests.testStock2_new);
        expect(scope.customResults).toContain(tests.testStock3_new);
        expect(scope.customResults).toEqual([tests.testStock2_new, tests.testStock3_new]);
      });
    });
  });
});